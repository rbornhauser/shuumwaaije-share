...
async function isBucketAllowed(
  ...
  const bucketRule = bucketPermissions[bucketName] as
    | BucketPermission
    | undefined;
  ...
  return rules.some((rule) =>
    matchesPermissionRule(rule, username, groups, path)
  );
}

export function R2Explorer(config?: R2ExplorerConfig) {
  extendZodWithOpenApi(z);
  config = config || ({} as R2ExplorerConfig);
  ...
  const bucketAccessMiddleware = async (
    c: AppContext,
    next: () => Promise<void>
  ) => {
